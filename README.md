...
| `site/index.html` `#rite` 底部 | 中式敬茶环节的**时段** | 「当日另备一袭中式礼服　恭候诸亲光临」 |
| `site/index.html` `#foot` | **双方家长姓名** | 「金 府　·　刘 府　谨订」（不知道名字时这是标准写法，不是占位符） |
...
思源宋体全量 15MB 上不了手机。脚本会扫描 HTML 里真正会显示的字（现在 371 个），
...
表单带真实 `action`，导航是真实 `<a href>`，折叠用原生 `<details>`。
